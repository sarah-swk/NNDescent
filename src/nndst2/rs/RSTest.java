...
public class RSTest extends Configured implements Tool {
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length != param) {
			System.out.println("Usage: RSTest <Input Path> <Output Path> <N> <K> <RHO>");
			System.exit(-1);
		}
		...
		status =ToolRunner.run(conf, new RSTest(), otherArgs);
		System.exit(status);
	} //main()
	...
	public int run(String[] args) throws Exception {
		...
		Job job =new Job(getConf(), "RS Test");
		job.setJarByClass(RSTest.class);
